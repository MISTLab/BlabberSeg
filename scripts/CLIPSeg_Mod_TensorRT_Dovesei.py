...
def seg_mod_trt(input_img, img):
    ...
    a3, a6, a9 = ort_session_0.run(['activations3', 'activations6', 'activations9'], 
                                   {'input_tensor': img_tensor})

    logits = []
    for c1, c2 in zip(conditionals[0], conditionals[1]):
        segmentation = ort_session_1.run(['segmentation'], 
                                         {'cond0': c1.half().view(1, 64).cpu().numpy(),
                                          'cond1': c2.half().view(1, 64).cpu().numpy(),
                                          'activations3': a3,
                                          'activations6': a6,
                                          'activations9': a9})

        logits.append(np.array(segmentation))
    
    logits = torch.squeeze(torch.Tensor(np.array(logits)), (1, 2))
    ...
    logitsCLIPSegModTRT.append(logits)
                    
    # End model computations
    time_spent.append(timer() - loop_start)
    
    segmentation_file = f"fp16_quant_dataset/segmentations/Mod_trt/segmentation_Original_Mod_TRT_{img.split('/')[-1].split('.')[0].split('-')[-1]}.png"
...
if method == 'img_rand_noise':
    ...
    for trial in range(total_trials):
        ...
        seg_mod_trt(input_img_rand, trial)
elif method == 'imgs':
...
# Accuracy comparison: Original DOVESEI FP32 vs CLIPSeg Mod TRT.
confmat = ConfusionMatrix(2) # background:0 and safe landing:1

for gt, test in zip(logits32, logitsCLIPSegModTRT):
