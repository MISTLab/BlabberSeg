...
def seg16(input_img, img):
    ...
    with torch.no_grad():
        ...
        logits = model16(inp_image, conditional = prompts, return_features = False, mask = None)[0]
    ...
    logits16.append(logits)
    
    # End model computations
    time_spent.append(timer() - loop_start)  
        
    segmentation_file = f"fp16_quant_dataset/segmentations/FP16/segmentation_Original_CLIPSeg_FP16_{str(img).split('/')[-1].split('.')[0].split('-')[-1]}.png"
    print(f"Saving {segmentation_file}...")
    Image.fromarray(logits).save(segmentation_file)
        
# Model setup - using OpenCLIP
model16 = CLIPDensePredT(version = 'ViT-B/16', reduce_dim = 64, complex_trans_conv = True, device = torch.device(device), openclip = True)
model16.load_state_dict(torch.load('weights/rd64-uni-refined.pth', map_location = torch.device(device)), strict = False);
model16.eval()
model16.cuda()
model16.half()
...
if method == 'img_rand_noise':
    ...
    for img in images:
        ...
        seg16(input_img_pil, img)
else:
    print("Invalid test method") 

print(f"Total time: {timer()-start:.6f}s")
print(f"Device: {torch.cuda.get_device_name(0)}, Total trials: {total_trials}, Mean: {np.asarray(time_spent).mean():0.6f}s, Std: {np.asarray(time_spent).std():0.6f}s")
...
# Accuracy comparison: Original DOVESEI FP32 vs Original DOVESEI FP16.
confmat = ConfusionMatrix(2) # background:0 and safe landing:1

for gt, test in zip(logits32, logits16):
