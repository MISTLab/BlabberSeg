...
from models.clipseg_mod import CLIPActivations, CLIPSegDecoder
import torch
...
def seg_mod(input_img, img):
    loop_start = timer()
    img_tensor = transform(input_img).unsqueeze(0).half()   
    ...
    with torch.no_grad():
        ...
        activations = activations_model(img_tensor)
        logits = []
        for c1, c2 in zip(conditionals[0], conditionals[1]):
            original_preds = np.array(seg_model(c1.half(), c2.half(), *activations).cpu())
            logits.append(original_preds)
        
    # Softmax
    logits = torch.Tensor(np.array(logits)).softmax(dim = 0).detach().cpu().numpy()
    logits = np.reshape(logits, (13, 1, 352, 352))   
    ...
    logitsCLIPSegMod.append(logits)  
         
    # End model computations
    time_spent.append(timer() - loop_start)    
        
    segmentation_file = f"fp16_quant_dataset/segmentations/Mod/segmentation_Original_Mod_{img.split('/')[-1].split('.')[0].split('-')[-1]}.png"
...
# Accuracy comparison: Original DOVESEI FP32 vs CLIPSeg Mod.
confmat = ConfusionMatrix(2) # background:0 and safe landing:1

for gt, test in zip(logits32, logitsCLIPSegMod):
