...
# HuggingFace model
def segHF(input_img, img):
    loop_start = timer()
    
    # Start model computations
    with torch.no_grad():
        inputs = processor(text = prompts, images = [input_img] * len(prompts), padding = True, return_tensors = "pt")
        time_spent_transform.append(timer() - loop_start)
        for k in inputs:
            if torch.cuda.is_available():
                inputs[k] = inputs[k].cuda()
        logits = modelHF(**inputs).logits
    ...
    logitsHF.append(logits)
                   
    # End model computations
    time_spent.append(timer() - loop_start)
    
    segmentation_file = f"fp16_quant_dataset/segmentations/HF/segmentation_Original_CLIPSeg_HF_{img.split('/')[-1].split('.')[0].split('-')[-1]}.png"
...
if 'cuda' in device:
    modelHF = CLIPSegForImageSegmentation.from_pretrained("CIDAS/clipseg-rd64-refined").cuda()
else:
    modelHF = CLIPSegForImageSegmentation.from_pretrained("CIDAS/clipseg-rd64-refined")
modelHF = modelHF.eval()
...
# Accuracy comparison: Original DOVESEI FP32 vs Original DOVESEI HF.
confmat = ConfusionMatrix(2) # background:0 and safe landing:1

for gt, test in zip(logits32, logitsHF):
