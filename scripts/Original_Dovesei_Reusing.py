...
with open("cond/conditionals_FP32.pkl", 'rb') as f: 
    ...
    if device == 'cuda':
         cond_from_clip_float = cond_from_clip_float.cuda()

def seg32_reuse(input_img, img):
    loop_start = timer()
    
    # Start model computations
    with torch.no_grad():
        inp_image = transform(input_img).unsqueeze(0)
        inp_image = torch.cat([inp_image] * cond_from_clip_float.shape[0])
        time_spent_transform.append(timer() - loop_start)
        if device == 'cuda':
            inp_image = inp_image.cuda()
        logits = model32(inp_image, conditional = cond_from_clip_float, return_features = False, mask = None)[0]
    ...
    logits32_reusing.append(logits) 
           
    # End model computations
    time_spent.append(timer() - loop_start)
    
    segmentation_file = f"fp16_quant_dataset/segmentations/Reusing/segmentation_Original_CLIPSeg_Reusing_{str(img).split('/')[-1].split('.')[0].split('-')[-1]}.png"
    print(f"Saving {segmentation_file}...")
    Image.fromarray(logits).save(segmentation_file)
  
# Model setup
model32 = CLIPDensePredT(version = 'ViT-B/16', reduce_dim = 64, device = torch.device(device), complex_trans_conv = True)
...
if method == 'img_rand_noise':
    ...
    for trial in range(total_trials):
        ...
        seg32_reuse(input_img_rand, trial)
elif method == 'imgs':
...
# Accuracy comparison: Original DOVESEI FP32 vs Original DOVESEI FP32 Reusing.
confmat = ConfusionMatrix(2) # background:0 and safe landing:1
for gt, test in zip(logits32, logits32_reusing):
